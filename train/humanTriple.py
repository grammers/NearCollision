...
def save_model_weights(epoch_num):
    model_file = path + 'trained_models/human_triple/humanTriple6s_' + str(epoch_num).zfill(3)
...
model_path = path + 'trained_models/vgg_on_voc' + str(800)
...
if os.path.exists('humanTriple6s'):
    shutil.rmtree('humanTriple6s')
logger = Logger('humanTriple6s', name='performance_curves')
...
model = model.cuda()
test_dir = path + 'h5_file/human_triple_test/'
train_dir = path +'h5_file/human_triple_train/'
...
for e in range(epochs):
    ...
    for test_file in os.listdir(test_dir):
        ...
        #print('test err = %s' % str(mean_err))
        hfp_test.close()
    # mean of the entire test setion
    print('mean err = %s' % str(mean_err/21.0))
    model.train() 

    f = open(path + 'trained_models/human_triple/loog.txt \n', 'a')
    f.write(str(e) + ' - ' + str(mean_err/21.0))
    f.close()
